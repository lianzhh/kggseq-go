...
func ReadBGZFLines(lj []byte, n int, currPos int, breakLine_ptr *Splinter) ([]byte, int) {
	lastPos := currPos
	for ; currPos < n; currPos++ {
		if lj[currPos] == 10 {
			if breakLine_ptr.marker != 0 {
				temp := breakLine_ptr.marker
				breakLine_ptr.marker = 0
				return append(breakLine_ptr.mx[:temp+1], lj[lastPos:(currPos+1)]...), currPos + 1
			} else {
				return lj[lastPos:(currPos + 1)], currPos + 1
			}

		}
	}
	copy(breakLine_ptr.mx[0:(currPos-lastPos)], lj[lastPos:(currPos-1)])
	breakLine_ptr.marker = currPos - lastPos
	return nil, n
